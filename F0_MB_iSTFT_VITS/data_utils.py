...
class TextAudioLoader(torch.utils.data.Dataset):
    ...
    def get_audio_text_pair(self, audiopath_and_text):
        ...
        text, text_lengths, lang = self.get_text(text)
        spec, wav = self.get_audio(audiopath)
        f0 = self.get_f0(wav)
        return (text, spec, wav, f0, lang)
    ...
    def get_text(self, text):
        ...
        text_norm, text_norm_lengths, lang = text_to_sequence(text)

        if self.add_blank:
            text_norm = commons.intersperse(text_norm, 0)

        text_norm = torch.LongTensor(text_norm).view(-1)
        return text_norm, text_norm_lengths, lang
...
class TextAudioCollate():
    ...
    def __call__(self, batch):
        ...
        if self.return_ids:
            return text_padded, text_lengths, spec_padded, spec_lengths, wav_padded, wav_lengths, ids_sorted_decreasing, f0_padded, lang_ids
        return text_padded, text_lengths, spec_padded, spec_lengths, wav_padded, wav_lengths, f0_padded, lang_ids
