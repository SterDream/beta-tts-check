...
try:
  ...
  from . import monotonic_alignment
  from . import modules
  ...
  import modules
  import attentions
  import monotonic_alignment
  import modules

from commons import init_weights, get_padding
from pqmf import PQMF 
from stft import TorchSTFT
from text.phonemize import to_IPA_converter
...
class TextEncoder(nn.Module):
  def __init__(self, n_vocab, out_channels, hidden_channels, filter_channels, n_heads, n_layers, kernel_size, p_dropout):
    ...
    # languages embedding
    self.num_langs = len(to_IPA_converter.g2p_map)
    self.lang_embedding = nn.Embedding(self.num_langs, hidden_channels)
  ...
  def forward(self, x, x_lengths, lang_id):
    ...
    if lang_id is not None:
      ...
      lang_emb = lang_emb.expand(-1, -1, x.size(2))
...
class SynthesizerTrn(nn.Module):
  ...
  def forward(self, x, x_lengths, y, y_lengths, lang_id=None, ref_mel=None, sid=None):
    ...
    x, m_p, logs_p, x_mask = self.enc_p(x, x_lengths, lang_id)
    ...
    with torch.no_grad():
      ...
      attn = monotonic_alignment.maximum_path(neg_cent, attn_mask.squeeze(1)).unsqueeze(1).detach()
