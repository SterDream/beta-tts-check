...
class Tokenizer:
    ...
    def to_tensor_for_inference(self, text):
        ipa, lang = self.g2p(text)
        tokens = self.ipa_tokenizer.tokenize("".join(ipa))
        ipa = self.vocab.encode(tokens)

        with torch.no_grad():
            x = torch.LongTensor(ipa).unsqueeze(0)
            x_lengths = torch.LongTensor([x.size(1)])
        return x, x_lengths, lang
...
if __name__ == "__main__":
    ...
    print(ids)
    x, x_lengths, lang = tokenizer.to_tensor("我毎天都喝珍珠奶茶!")
    print(x, x_lengths, lang)
