...
def run(rank, n_gpus, hps):
    ...
    train_sampler = DistributedBucketSampler(
      ...
      [32, 300, 400, 500, 600, 700, 800, 900, 1000, 1200, 1500],
      num_replicas=n_gpus,
      rank=rank,
      shuffle=True
    )
...
def train_and_evaluate(
  ...
  for batch_idx, (x, x_lengths, spec, spec_lengths, y, y_lengths, f0_gt, lang) in enumerate(train_loader):
    # CUDA
    ...
    with autocast(enabled=hps.train.fp16_run):
      ...
      (z, z_p, m_p, logs_p, m_q, logs_q), f0_pred = net_g(x, x_lengths, spec, spec_lengths, lang)
    ...
    if loss_disc_all is not None and loss_disc_all.item() != 0:
    ...
    if rank==0:
      if global_step % hps.train.log_interval == 0 and not speedup:
...
def evaluate(hps, generator, eval_loader, writer_eval):
    ...
    with torch.no_grad():
      for batch_idx, (x, x_lengths, spec, spec_lengths, y, y_lengths, f0_gt, lang) in enumerate(eval_loader):
        ...
        for use_sdp in [True, False]:
          y_hat, y_hat_mb, attn, mask, *_ = generator.module.infer(x, x_lengths, lang, max_len=1000)
